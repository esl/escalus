...
init([Args, Owner]) ->
    ...
    Pid = self(),
    wsecli:onopen(fun() -> Pid ! opened end),
    wsecli:onerror(fun(Reason) -> Pid ! {error, Reason} end),
    wsecli:onmessage(fun(Type, Data) -> Pid ! {Type, Data} end),
    wsecli:onclose(fun(_Msg) -> Pid ! tcp_closed end),
